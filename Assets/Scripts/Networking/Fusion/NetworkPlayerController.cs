...
namespace Project.Networking.Fusion
{
    ...
    public class NetworkPlayerController : NetworkBehaviour
    {
        ...
        public override void FixedUpdateNetwork()
        {
            ...
            Vector2 move = ConvertInputToWorld(input.Move);
        ...
        private Vector2 ConvertInputToWorld(Vector2 rawInput)
        {
            if (rawInput.sqrMagnitude <= 0.0001f)
                return Vector2.zero;

            int presetIndex = GetCameraPresetIndex();
            GetCameraPreset(presetIndex, out _, out Vector3 euler);

            Quaternion yawRotation = Quaternion.Euler(0f, euler.y, 0f);
            Vector3 world = yawRotation * new Vector3(rawInput.x, 0f, rawInput.y);
            Vector2 worldInput = new Vector2(world.x, world.z);
